...
class ZenhomTombController extends Controller
{
    public function index()
    {
        ...
        return view('المقابر.زينهم.index', compact('region', 'tombs', 'tombData'));
    }
    ...
    public function showRoom($tombId, $roomId)
    {
        ...
        $tombName = $tomb->name;
        return view('المقابر.زينهم.room', compact('region', 'room', 'deceased', 'tombName'));
    }
    public function deleteDeceased($id)
    {
        ...
        if ($deceased) {
            ...
            $deceased->delete();
            return redirect()->route('zenhom.index')->with('تم الحذف بنجاح');
        }
        return redirect()->route('zenhom.index')->withErrors('خطأ أثناء الحذف');
    }
    public function updateDeceased(Request $request)
    {
        ...
        if ($deceased) {
            ...
            if ($update) {
                return redirect()->route('zenhom.index')->with('success', 'تم التعديل بنجاح');
            }
        }
        return redirect()->route('zenhom.index')->withErrors('حدث خطأ أثناء التحديث');
    }
}
