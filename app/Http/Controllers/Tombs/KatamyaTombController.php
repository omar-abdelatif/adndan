...
class KatamyaTombController extends Controller
{
    public function index()
    {
        ...
        return view('المقابر.القطامية.index', compact('region', 'tombs', 'tombData'));
    }
    ...
    public function deleteDeceased($id)
    {
        ...
        if ($deceased) {
            ...
            $deceased->delete();
            return redirect()->route('katamya.index')->with('تم الحذف بنجاح');
        }
        return redirect()->route('katamya.index')->withErrors('خطأ أثناء الحذف');
    }
    public function updateDeceased(Request $request)
    {
        ...
        return redirect()->route('katamya.index')->withErrors('حدث خطأ أثناء التحديث');
    }
}
