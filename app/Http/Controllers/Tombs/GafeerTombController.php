...
class GafeerTombController extends Controller
{
    ...
    public function deleteDeceased($id)
    {
        ...
        if ($deceased) {
            ...
            $deceased->delete();
            return redirect()->route('gafeer.index')->with('تم الحذف بنجاح');
        }
        return redirect()->route('gafeer.index')->withErrors('خطأ أثناء الحذف');
    }
    public function updateDeceased(Request $request)
    {
        ...
        if ($deceased) {
            ...
            if ($update) {
                return redirect()->route('gafeer.index')->with('success', 'تم التعديل بنجاح');
            }
        }
        return redirect()->route('gafeer.index')->withErrors('حدث خطأ أثناء التحديث');
    }
}
